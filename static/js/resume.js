...
function display_resume(resume){
  $('#NAME_DIV').html(resume.name); $('#LOCATION_DIV').html(resume.location); if('EMAIL_DIV' in resume) $('#email').html('<a href="'+resume.email + '">' + resume.email + '</a>');
  if('WEBSITE_DIV' in resume) $('#website').html('<a href="'+resume.website + '">' + resume.website + '</a>');
  if('GITHUB_DIV' in resume) $('#github').html('<a href="'+resume.github + '">' + resume.github + '</a>');
  if('LINKEDIN_DIV' in resume) $('#linkedin').html('<a href="'+resume.linkedin + '">' + resume.linkedin + '</a>');

  //EDUCATION
  $.each(resume.Education, function(index, degree){
    $('#EDUCATION_DIV').prepend('<div class="row text-white">' + degree.degree + ' - ' + degree.Institution + '</div>');
    if(['Cumulative GPA'] in degree )$('#EDUCATION_DIV').prepend('<div class="row"><li>Cumulative GPA: ' + degree['Cumulative GPA'] + '</li></div>');
  });

  //KNOWLEDGE
  $.each(resume.Knowledge, function(index, knowledge){
    $('#KNOWLEDGE_DIV').prepend('<div class="text-white col">' + knowledge + '</div>');
  });

  //Programming Languages
  $.each(resume['Programming Languages'], function(index, language){
    $('#LANGUAGES_DIV').prepend('<div class="text-white col">' + language + '</div>');
  });

  //Frameworks
  $.each(resume.Frameworks, function(index, framework){
    $('#FRAMEWORKS_DIV').prepend('<div class="text-white col">' + framework + '</div>');
  });
...
function add_edit_btns(){
  $('#EDUCATION_SECTION_DIV').append('<div id="EDUCATION_DIV_BTN" class="row"><button class="btn btn-success" type="button" onclick="add_degree()">Add</button></div>');
  $('#KNOWLEDGE_SECTION_DIV').append('<div id="KNOWLEDGE_DIV_BTN" class="container row"><button class="btn btn-success" type="button" onclick="add_knowledge()">Add</button></div>');
  $('#PROGRAMMING_LANGUAGE_DIV').append('<div id="LANGUAGES_DIV_BTN" class="container row"><button class="btn btn-success" type="button" onclick="add_language()">Add</button></div>');
  $('#FRAMEWORKS_SECTION_DIV').append('<div id="FRAMEWORKS_DIV_BTN" class="container row"><button class="btn btn-success" type="button" onclick="add_framework()">Add</button></div>');
  $('#DATABASES_SECTION_DIV').append('<div id="DATABASE_DIV_BTN" class="container row"><button class="btn btn-success" type="button" onclick="add_db_type()">Add Database Type</button></div>');
  $('#OPERATING_SYSTEMS_DIV').append('<div id="OS_DIV_BTN" class="container row"><button class="btn btn-success" type="button" onclick="add_os()">Add</button></div>');
  $('#CLASS_PROJECTS_DIV').append('<div id="CPROJ_DIV_BTN" class="container row"><button class="btn btn-success" type="button" onclick="add_cproj()">Add</button></div>');
  $('#RESEARCH_DIV').append('<div id="RSCH_DIV_BTN" class="container row"><button class="btn btn-success" type="button" onclick="add_rsch()">Add</button></div>');
  $('#PPROJ_SECTION_DIV').append('<div id="PPROJ_DIV_BTN" class="container row"><button class="btn btn-success" type="button" onclick="add_pproj()">Add</button></div>');
  $('#EMPLOYMENT_DIV').append('<div id="WORK_DIV_BTN" class="container row"><button class="btn btn-success" type="button" onclick="add_job()">Add</button></div>');
  $('#END').append('<div id="SAVE_DIV_BTN" class="container row"><button class="btn-lg btn-success" type="button" onclick="save()">SAVE RESUME</button></div>');
}
...
function add_top_section(resume = {'name': 'Name', 'location': 'Location', 'email': 'email', 'website': 'Website', 'github': 'GitHub', 'linkedin': 'Linkedin'}){
  ...
  location_fld += '<input type="text" class="form-control" id="LOCATION" aria-describedby="name" value="' + resume.location + '"></div>';
  $('#LOCATION_DIV').prepend(location_fld);

  var email_fld = '<label class="control-label" for="email">Email</label>';
  email_fld += '<input type="text" class="form-control" id="EMAIL" aria-describedby="email" value="' + resume.email + '">';
  $('#EMAIL_DIV').prepend(email_fld);

  var website_fld = '<label class="control-label" for="website">Website</label>';
  website_fld += '<input type="text" class="form-control" id="WEBSITE" aria-describedby="website" value="' + resume.website + '">';
  $('#WEBSITE_DIV').prepend(website_fld);

  var github_fld = '<label class="control-label" for="github">Github</label>';
  github_fld += '<input type="text" class="form-control" id="GITHUB" aria-describedby="github" value="' + resume.github + '">';
  $('#GITHUB_DIV').prepend(github_fld);

  var linkedin_fld = '<label class="control-label" for="linkedin">LinkedIn</label>';
  linkedin_fld += '<input type="text" class="form-control" id="LINKEDIN" aria-describedby="linkedin" value="' + resume.linkedin + '">';
...
function add_db_type(db='Database Type'){
  ...
  dbtype += '<button class="btn form-control btn-success add_new_db_instance" type="button" >+</button><div id="' + dbt_id + '" class="row container border-bottom"></div></div><br>';
...
function add_job(job={'Employer':'Employer','from':'From','to':'To','Position':'Job Title','Duties':'Job Duties'}){
 // console.log(JSON.stringify(job, null, 2));8
  var job_div = '<div class="row form-inline border-secondary WORK_HISTORY">';
  ...
  job_div += '<input type="text" class="form-control JOB-TITLE" id="title-' + job_ctr + '" aria-describedby="title" value="'+ job.Position +'">';
